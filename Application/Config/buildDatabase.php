...
END;
        ...
        print '<br>Table `Chapters` already exists</p>';
    } catch (PDOException $e) {
        print '<br><p style="color: red">Creating `Chapters`</p>';
        $sql = <<<END
        $head
...
CREATE TABLE `Chapters` (
  `ChapterNumber` int(11) NOT NULL AUTO_INCREMENT,
  `ChapterTitle` text COLLATE utf8mb4_general_ci,
  PRIMARY KEY (`ChapterNumber`),
  UNIQUE KEY `Chapters_ChapterNumber_uindex` (`ChapterNumber`)
...
END;
        ...
        print $sql . '<br>';
        $db->exec($sql) === false and die(print_r($db->errorInfo(), true));
        print '<br><p style="color: green">Table `Chapters` Created</p>';
    }try {
        $db->prepare('SELECT 1 FROM Questions LIMIT 1;')->execute();
        print '<br>Table `Questions` already exists</p>';
    } catch (PDOException $e) {
        print '<br><p style="color: red">Creating `Questions`</p>';
        $sql = <<<END
        $head
...
END;
        ...
        print $sql . '<br>';
        $db->exec($sql) === false and die(print_r($db->errorInfo(), true));
        print '<br><p style="color: green">Table `Questions` Created</p>';
    }try {
        $db->prepare('SELECT 1 FROM Sections LIMIT 1;')->execute();
        print '<br>Table `Sections` already exists</p>';
    } catch (PDOException $e) {
        print '<br><p style="color: red">Creating `Sections`</p>';
        $sql = <<<END
        $head
...
CREATE TABLE `Sections` (
  `ChapterNumber` text COLLATE utf8mb4_general_ci,
  `SectionTitle` text COLLATE utf8mb4_general_ci,
  `SectionNumber` text COLLATE utf8mb4_general_ci
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
END;
        ...
        print $sql . '<br>';
        $db->exec($sql) === false and die(print_r($db->errorInfo(), true));
        print '<br><p style="color: green">Table `Sections` Created</p>';
    }try {
